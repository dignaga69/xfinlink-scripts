...
px = px[(px["m"] >= pd.Period("2005-01")) & (px["m"] <= pd.Period("2024-12"))]
...
# the company's own split-adjusted price. Company-months where the two disagree
# by more than 0.5 in logs leave the panel, both universes alike.
disagree = (np.log1p(R) - np.log(A).diff()).abs() > 0.5
R = R.mask(disagree)
...
names = px.drop_duplicates("entity_id").set_index("entity_id")["ticker"]
best, worst = R.stack().idxmax(), R.stack().idxmin()
...
print()
print(f"Companies across the 20 January rosters {len(set().union(*pit.values())):5d}")
print(f"Companies on the roster today           {len(current):5d}")
print(f"January 2005 members still on it today  {len(pit[2005] & current):5d} "
      f"of {len(pit[2005])}")
print(f"Panel   {R.shape[0]} months x {R.shape[1]} companies, "
      f"{int(R.notna().sum().sum()):,} company-months, "
      f"{int(disagree.sum().sum())} set aside by the agreement screen")
print(f"Range   {R.max().max()*100:.0f}% ({names[best[1]]}, {best[0]}) to "
      f"{R.min().min()*100:.1f}% ({names[worst[1]]}, {worst[0]})")
print()
...
W = books(R.clip(R.quantile(0.01, axis=1), R.quantile(0.99, axis=1), axis=0))
wp = (1 + W["pit"]).prod() ** (12 / n) - 1
ws = (1 + W["sur"]).prod() ** (12 / n) - 1
print(f"\nRobustness: clipping each month's cross-section at the 1st and 99th "
      f"percentiles\n  gives {wp * 100:.2f}% against {ws * 100:.2f}%, a gap of "
      f"{(ws - wp) * 100:.2f}pp.")
...
x = P.index.year + (P.index.month - 1) / 12
